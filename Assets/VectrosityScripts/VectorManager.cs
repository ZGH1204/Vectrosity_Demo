...
public class VectorManager {
    ...
    /// <param name="makeBounds">" The optional makeBounds by default creates an invisible bounds mesh for the gameObject’s mesh filter, so that OnBecameInvisible and OnBecameInvisible will still work, which allows optimizations for Visibility.Dynamic and Visibility.Static.If set to false, then the gameObject’s mesh is not replaced by a bounds mesh.One bounds mesh is created for each VectorLine name, so VectorLines that share a name will also share a bounds mesh."</param>
