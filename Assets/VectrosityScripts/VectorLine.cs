...
namespace Vectrosity
{
    ...
    /// <summary>
    /// Jions.none 作为默认的划线，尤其是细线。
    ///   Jions.Fill 线有角度，尤其线比较密集的时候没有贴图的地方。
    ///   以上两种仅仅使用在LineType.Continuous情况下。
    ///  Joins.Weld 适用于LineType.Discrete
    /// </summary>
    ...
    /// <summary>
    /// 可见性
    /// visibility can be Visibility.Dynamic, Visibility.Static, Visibility.Always, or Visibility.None. 
    /// Visibility.Dynamic：causes the vector line to always be drawn when gameObject is visible to a camera, using the object’s transform.
    /// Visibility.Static： only draws the vector line if the camera moves and the gameObject is visible to a camera, and is for objects that never move, since the object’s transform is only used once when the 3D  vector line is initialized.
    /// Visibility.Always:  causes the vector line to be drawn every frame and has no optimizations.
    /// Visibility.None :   doesn’t add any visibility components, and removes any Visibility scripts if ObjectSetup has been called on this gameObject before.
    /// </summary>
    ...
    public class VectorLine
    {
        ...
        private bool m_fillObjectSet = false;
        ...
        private static Vector3 v3zero = Vector3.zero;   // Faster than using Vector3.zero since that returns a new instance
        ...
        private static Canvas _canvas3D;
        ...
        private static void SetCanvas3D ()
        {
            SetCamera3D ();
            if (!cam3D)
            {
        ...
        private static string[] functionNames = { "VectorLine.SetColors: Length of color", "VectorLine.SetWidths: Length of line widths", "MakeCurve", "MakeSpline", "MakeEllipse" };

        private enum FunctionName
        { SetColors, SetWidths, MakeCurve, MakeSpline, MakeEllipse }
